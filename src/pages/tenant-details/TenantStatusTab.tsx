...
const TenantStatusTab = ({ tenantId }: TenantStatusTabProps) => {
  ...
  const [shouldAutoSync, setShouldAutoSync] = useState(true)
  ...
  const {
    ...
    error: statusError,
    dataUpdatedAt,
  } = useGetUserTenantStatus(tenantId, shouldAutoSync ? 10000 : 0)

  useEffect(() => {
    if (!statusData) {
      return
    }
    const autoJobs = statusData.status.jobs.filter(
      (j) => j.name !== 'CHECK_READINESS' && j.mode !== 'MANUAL',
    )
    setShouldAutoSync(autoJobs.some((j) => j.status !== 'COMPLETED'))
  }, [statusData])
  ...
  return (
    <div className="flex flex-col max-w-[1040px] mx-auto mt-1">
      {shouldAutoSync && (
        <div className="flex items-center justify-end mb-3">
          <div className="flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs text-emerald-700">
            <span className="relative flex size-2 flex-shrink-0 me-0.5">
