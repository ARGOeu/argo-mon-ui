...
import { useCheckSlugQuery } from '@/hooks/usePages'
...
const BuildConfigTab = ({
  ...
  const { data: slugCheckData } = useCheckSlugQuery(
    tenantId,
    debouncedSlug,
    !isEditMode && !!tenantId && !!debouncedSlug,
  )

  const slugExists = slugCheckData?.exist === true && slug === debouncedSlug
  ...
  useEffect(() => {
    onSlugExistsChange?.(slugExists)
  }, [slugExists, onSlugExistsChange])
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-4 lg:gap-6">
        ...
        <div className="bg-white border border-line rounded-lg p-6 space-y-2">
          ...
          <div>
            ...
            <input
              type="text"
              className={`w-full${slugExists ? ' !border-red-500' : ''}`}
